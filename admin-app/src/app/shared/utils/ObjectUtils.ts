 ...
 */
function replaceCharacter(source, replacementScheme) {
...
function hiddenToLongStr(initName,length):string{
  if(isNotEmpty(initName)){
    if(initName.length >length){
      return initName.substr(0,length) + '...';
    }else{
      return initName;
    }
  }
