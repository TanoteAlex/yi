...
export class ListFreightTemplateConfigComponent implements OnInit {
  ...
  getData() {
    this.loading = true;
    this.freightTemplateConfigService.queryForSupplier(this.pageQuery).subscribe(response => {
  ...
  remove(freightTemplateConfigId) {
    this.freightTemplateConfigService.removeByIdForSupplier(freightTemplateConfigId).subscribe(response => {
