...
//        this.pageQuery.clearFilter();
    ...
    routeDate() {
        ...
        this.startTimes = date1[0] + "-" + date1[1] + "-" + date1[2]
        this.endTimes = date1[0] + "-" + date1[1] + "-" + date1[2] + " " + "23:59:59"
    ...
    downloadExcel(reset: boolean = false): void {
        ...
        this.configPageQuery(this.pageQuery);
        this.saleOrderService.download("GET", "销售订单-" + new Date().getTime() + ".xls", "/saleOrder/exportExcelForSupplier", this.pageQuery);
    }
    ...
    getOrderNumForSupplier() {
        //查询各状态订单数量
        this.saleOrderService.getOrderNumForSupplier().subscribe(response => {
    ...
    goUpdateOrderState(id, orderState) {
        this.saleOrderService.updateOrderStateForSupplier(id, orderState).subscribe(response => {
            if (response.result == SUCCESS) {
                this.getData();
            } else {
                this.msg.error(response.message ? response.message : '请求失败');
...
//        this.pageQuery.addOnlyFilter('orderState', 5, 'neq');
        return pageQuery;
    }
