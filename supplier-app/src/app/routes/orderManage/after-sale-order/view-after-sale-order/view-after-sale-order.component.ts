...
export class ViewAfterSaleOrderComponent implements OnInit {
  ...
  getById(objId) {
    this.afterSaleOrderService.getByIdForSupplier(objId).subscribe(response => {
      if (response.result == SUCCESS) {
        this.afterSaleOrder = response.data;
        this.pageQuery.addOnlyOneFilter("afterSaleOrder.id", this.afterSaleOrder.id, "eq")
        this.afterSaleProcessService.queryForSupplier(this.pageQuery).subscribe(response => {
  ...
  returnRefund(id, type) {
    ...
    this.afterSaleProcessService.confirmReturnForSupplier(formValue.afterSaleProcess).subscribe(response => {
      if (response.result == SUCCESS) {
